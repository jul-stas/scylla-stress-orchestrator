...
cluster_private_ips = env['cluster_private_ips'][:-1]
cluster_string = ",".join(cluster_private_ips)
new_node_private_ip = env['cluster_private_ips'][-1]
cluster_public_ips = env['cluster_public_ips'][:-1]
new_node_public_ip = env['cluster_public_ips'][-1]
...
# Start Scylla/Cassandra nodes (except one to be started later)
...
# Background load
background_load = cs.async_stress(f'mixed ratio\\(write=1,read=1\\) duration=30m cl=QUORUM -pop dist=UNIFORM\\(1..{ROW_COUNT}\\) -log hdrfile=profile.hdr -graph file=report.html title=benchmark revision=benchmark-0 -mode native cql3 -rate "threads=100 fixed={BACKGROUND_LOAD_OPS // loadgenerator_count}/s" -node {cluster_string}')

add_node_start = datetime.now()

# Start a single new Scylla/Cassandra node
if props['cluster_type'] == 'scylla':
    s = Scylla([new_node_public_ip], [new_node_private_ip], all_private_ips[0], props)
    s.start()
else:
    cassandra = Cassandra([new_node_public_ip], [new_node_private_ip], all_private_ips[0], props)
    cassandra.start()

add_node_end = datetime.now()

print("Run ended at:", datetime.now().strftime("%H:%M:%S"))
print("Adding a single node took:", (add_node_end - add_node_start).total_seconds(), "seconds.")

background_load.join()
print("Background load ended at:", datetime.now().strftime("%H:%M:%S"))